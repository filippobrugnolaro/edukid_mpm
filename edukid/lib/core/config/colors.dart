import 'package:flutter/material.dart';

const white = Color(0xFFFFFFFF);
const black = Color(0xFF000000);
const red = Color.fromARGB(255, 204, 45, 45);
const orange = Color.fromARGB(255, 255, 132, 0);
const lightOrange = Color.fromARGB(255, 254, 230, 206);
const yellow = Color.fromARGB(255, 255, 225, 0);
const green = Color.fromARGB(255, 63, 188, 46);
const blue = Color.fromARGB(255, 22, 145, 221);
const lightblue = Color.fromARGB(255, 84, 224, 255);
const purple = Color(0xFF800080);
const pink = Color.fromARGB(255, 255, 209, 217);
const fucsia = Color.fromARGB(255, 234, 72, 177);
const grey = Color.fromARGB(255, 150 , 150, 150);
const transparent = Color(0x00000000);
